...
//                print_r($where);
                ...
                if (isset($wherein)) {
//                    echo "<pre>";
//                    print_r($whereinoptions);
//                    exit('--'.$wherein);
                    $this->db->where_in($wherein, $whereinoptions);
                }
                if (isset($wherenotin)) {
...
//return $this->db->affected_rows();
    ...
    public function _insertCustomer($data) {
        $table = "t3customers";
        $this->db->insert($table, $data);
    }

    public function get_user_login($user_name, $password) {
        $condition['username'] = $user_name;
        $condition['password'] = $password;
        return $this->get_by_condition('t1users', $condition);
    }

// get row with condition in specific table
    public function get_by_condition($tblname, $condition) {
    ...
    public function excute_query($qry) {
        $this->db->query($qry);
    }

    public function getkey_value($query) {//"select bhca as capacity_value,bsc_name from bsc where v_name='$vendor'"
